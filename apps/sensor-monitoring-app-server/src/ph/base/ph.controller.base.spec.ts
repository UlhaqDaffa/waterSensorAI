...
const service = {
  ...
  phs: () => FIND_MANY_RESULT,
  ph: ({ where }: { where: { id: string } }) => {
    switch (where.id) {
      case existingId:
...
describe("PH", () => {
  ...
  test("GET /phs/:id non existing", async () => {
    await request(app.getHttpServer())
      .get(`${"/phs"}/${nonExistingId}`)
  ...
  test("GET /phs/:id existing", async () => {
    await request(app.getHttpServer())
      .get(`${"/phs"}/${existingId}`)
      .expect(HttpStatus.OK)
      .expect({
  ...
  test("POST /phs existing resource", async () => {
